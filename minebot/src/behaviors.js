...
const GOAL_SEQUENCE =
[
    {x: -715, y: 63, z: 145, type: 'bridge_start'},
    {x: -735, y: 63, z: 145, type: 'bridge_end'},
    {x: -715, y: 63, z: 145, type: 'bridge_start'},
    {x: -700, y: 63, z: 145, type: 'checkpoint'},
...
class AutonomousBot
{
    ...
    /**
     * @brief Handles chest management (approach, open, collect, close, report)
     */
    async handleManagingChest()
    {
        try
        {
            const chestPos = new Vec3(this.chest.x, this.chest.y, this.chest.z);
            
            // Look at the chest before opening
            await this.bot.lookAt(chestPos);
    ...
    async handleMining()
    {
        try
        {
            ...
            const nextBlock = this.actions.block_at(pos.x, pos.y - 1, pos.z);
